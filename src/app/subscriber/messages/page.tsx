    ...
    <div style={{ flex: 1, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '40px' }}>
      <div style={{ textAlign: 'center', maxWidth: '300px' }}>
        ...
        <p style={{ fontSize: '13px', color: 'var(--text-3)', lineHeight: 1.5 }}>
          Follow a creator to start chatting with them directly.
        </p>
      </div>
    </div>
  );
...
export default function SubscriberMessagesPage() {
  ...
  const [active, setActive] = useState(DEMO_CONVERSATIONS[0]?.id ?? "");
  ...
  const showEmptyState = DEMO_CONVERSATIONS.length === 0;
